...
class SaturnChunkGenerator private constructor (
    ...
    companion object {
        val CODEC: Codec<SaturnChunkGenerator> = RecordCodecBuilder.create<SaturnChunkGenerator>(
            Function<RecordCodecBuilder.Instance<SaturnChunkGenerator>, App<RecordCodecBuilder.Mu<SaturnChunkGenerator>, SaturnChunkGenerator>> { instance: RecordCodecBuilder.Instance<SaturnChunkGenerator> ->
                instance.group(
                    BiomeSource.CODEC.fieldOf("biome_source").forGetter(SaturnChunkGenerator::getBiomeSource),
                    Codec.LONG.fieldOf("seed").stable().forGetter(SaturnChunkGenerator::genSeed),
                    ChunkGeneratorSettings.REGISTRY_CODEC.fieldOf("settings")
                        .forGetter { Supplier { it.settings() } },
                    RegistryLookupCodec.of(Registry.BIOME_KEY).forGetter(SaturnChunkGenerator::biomeRegistry)
                )
                    .apply(
                        instance,
                        instance.stable(Function4 { biomeSource, seed, genSettings, registry ->
                            SaturnChunkGenerator(
                                biomeSource,
                                seed,
                                { genSettings.get() },
                                registry
