...
class JupiterChunkGenerator private constructor (
    ...
    companion object {
        val CODEC: Codec<JupiterChunkGenerator> = RecordCodecBuilder.create<JupiterChunkGenerator>(
            Function<RecordCodecBuilder.Instance<JupiterChunkGenerator>, App<RecordCodecBuilder.Mu<JupiterChunkGenerator>, JupiterChunkGenerator>> { instance: RecordCodecBuilder.Instance<JupiterChunkGenerator> ->
                instance.group(
                    BiomeSource.CODEC.fieldOf("biome_source").forGetter(JupiterChunkGenerator::getBiomeSource),
                    Codec.LONG.fieldOf("seed").stable().forGetter(JupiterChunkGenerator::genSeed),
                    ChunkGeneratorSettings.REGISTRY_CODEC.fieldOf("settings")
                        .forGetter { Supplier { it.settings() } },
                    RegistryLookupCodec.of(Registry.BIOME_KEY).forGetter(JupiterChunkGenerator::biomeRegistry)
                )
                    .apply(
                        instance,
                        instance.stable(Function4 { biomeSource, seed, genSettings, registry ->
                            JupiterChunkGenerator(
                                biomeSource,
                                seed,
                                { genSettings.get() },
                                registry
