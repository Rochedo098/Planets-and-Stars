...
class NeptuneChunkGenerator private constructor (
    ...
    companion object {
        val CODEC: Codec<NeptuneChunkGenerator> = RecordCodecBuilder.create<NeptuneChunkGenerator>(
            Function<RecordCodecBuilder.Instance<NeptuneChunkGenerator>, App<RecordCodecBuilder.Mu<NeptuneChunkGenerator>, NeptuneChunkGenerator>> { instance: RecordCodecBuilder.Instance<NeptuneChunkGenerator> ->
                instance.group(
                    BiomeSource.CODEC.fieldOf("biome_source").forGetter(NeptuneChunkGenerator::getBiomeSource),
                    Codec.LONG.fieldOf("seed").stable().forGetter(NeptuneChunkGenerator::genSeed),
                    ChunkGeneratorSettings.REGISTRY_CODEC.fieldOf("settings")
                        .forGetter { Supplier { it.settings() } },
                    RegistryLookupCodec.of(Registry.BIOME_KEY).forGetter(NeptuneChunkGenerator::biomeRegistry)
                )
                    .apply(
                        instance,
                        instance.stable(Function4 { biomeSource, seed, genSettings, registry ->
                            NeptuneChunkGenerator(
                                biomeSource,
                                seed,
                                { genSettings.get() },
                                registry
