...
class UranusChunkGenerator private constructor (
    ...
    companion object {
        val CODEC: Codec<UranusChunkGenerator> = RecordCodecBuilder.create<UranusChunkGenerator>(
            Function<RecordCodecBuilder.Instance<UranusChunkGenerator>, App<RecordCodecBuilder.Mu<UranusChunkGenerator>, UranusChunkGenerator>> { instance: RecordCodecBuilder.Instance<UranusChunkGenerator> ->
                instance.group(
                    BiomeSource.CODEC.fieldOf("biome_source").forGetter(UranusChunkGenerator::getBiomeSource),
                    Codec.LONG.fieldOf("seed").stable().forGetter(UranusChunkGenerator::genSeed),
                    ChunkGeneratorSettings.REGISTRY_CODEC.fieldOf("settings")
                        .forGetter { Supplier { it.settings() } },
                    RegistryLookupCodec.of(Registry.BIOME_KEY).forGetter(UranusChunkGenerator::biomeRegistry)
                )
                    .apply(
                        instance,
                        instance.stable(Function4 { biomeSource, seed, genSettings, registry ->
                            UranusChunkGenerator(
                                biomeSource,
                                seed,
                                { genSettings.get() },
                                registry
